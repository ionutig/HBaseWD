 ...
 * @author Alex Baranau
 */
public abstract class HBaseWdTestBase {
  ...
  public HBaseWdTestBase(AbstractRowKeyDistributor keyDistributor) {
  ...
  public void testSimpleScanBounded() throws IOException {
    ...
    testSimpleScanImpl(origKeyPrefix, scan, 500, 500, seekIntervalMinValue, seekIntervalMaxValue);
  }
  ...
  public void testSimpleScanUnbounded() throws IOException {
    long origKeyPrefix = System.currentTimeMillis();
    testSimpleScanImpl(origKeyPrefix, new Scan(), 500, 500, 0, 999);
  }
  ...
  public void testMapReduceBounded() throws IOException, InterruptedException, ClassNotFoundException {
    ...
    byte[] stopKey = Bytes.toBytes(origKeyPrefix + seekIntervalMaxValue + 1);
    Scan scan = new Scan(startKey, stopKey);
    testMapReduceImpl(origKeyPrefix, scan, 500, 500, seekIntervalMinValue, seekIntervalMaxValue);
  }
  ...
  public void testMapReduceUnbounded() throws IOException, InterruptedException, ClassNotFoundException {
    long origKeyPrefix = System.currentTimeMillis();
    testMapReduceImpl(origKeyPrefix, new Scan(), 500, 500, 0, 999);
  }

  private int writeTestData(long origKeyPrefix, int numRows, int rowKeySeed, int seekIntervalMinValue, int seekIntervalMaxValue) throws IOException {
  ...
  private void testSimpleScanImpl(long origKeyPrefix, Scan scan, int numValues, int startWithValue, int seekIntervalMinValue, int seekIntervalMaxValue) throws IOException {
    int valuesCountInSeekInterval = writeTestData(origKeyPrefix, numValues, startWithValue, seekIntervalMinValue, seekIntervalMaxValue);
  ...
  private void testMapReduceImpl(long origKeyPrefix, Scan scan, int numValues, int startWithValue, int seekIntervalMinValue, int seekIntervalMaxValue) throws IOException, InterruptedException, ClassNotFoundException {
    int valuesCountInSeekInterval = writeTestData(origKeyPrefix, numValues, startWithValue, seekIntervalMinValue, seekIntervalMaxValue);

    // Reading data
    Configuration conf = testingUtility.getConfiguration();
//    Configuration conf = HBaseConfiguration.create();
//    conf.setInt("hbase.zookeeper.property.clientPort", 21818);
    Job job = new Job(conf, "testMapReduceImpl()-Job");
